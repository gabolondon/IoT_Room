...
const char * ssid = "TP-LINK_PISO 1";
const char * claveWifi = "@VT.830504734@";
...
void loop() {
 ...
 delay(10);
 if (millis() - timerEnvioDatos > 5000) { // Se utiliza un timer de 5 segundos (5000 ms)
 timerEnvioDatos=millis();
 float h = dht.readHumidity();
 float t = dht.readTemperature();
 if (isnan(h) || isnan(t)) { //se verifica si leyó desde el sensor DHT 
